...
namespace quantt
{
	template <class Key, class Value, class Comp_less = std::less<Key>, class Allocator = std::allocator<std::pair<Key, Value>>, template <class...> class Array = std::vector>
	class flat_map
	{
	public:
		using key_type = Key;
		...
		public:
			...
			// explicit iterator(typename content_t::iterator &&in) : content_t::iterator(std::move(in)) {}
			iterator operator+(std::ptrdiff_t in) { return iterator(content_t::iterator::operator+(in).base()); }
			iterator operator-(std::ptrdiff_t in) { return iterator(content_t::iterator::operator-(in).base()); }
			using content_t::iterator::operator=;
		}; //require random access.
		struct const_iterator : public content_t::const_iterator
		{
		public:
			const_iterator(const typename content_t::iterator &) = delete;
			...
			// explicit const_iterator(typename content_t::const_iterator &&in) : content_t::const_iterator(std::move(in)) {}
			// explicit const_iterator(typename content_t::iterator &&in) : content_t::const_iterator(std::move(in)) {}
			const_iterator(const iterator &in) : content_t::const_iterator(in) {}
			const_iterator operator+(std::ptrdiff_t in) { return const_iterator(content_t::const_iterator::operator+(in).base()); }
			const_iterator operator-(std::ptrdiff_t in) { return const_iterator(content_t::const_iterator::operator-(in).base()); }
		}; //require random access.
		...
		iterator end()
		{
			return iterator(content.end().base());
		}
		const_iterator cbegin() const
		{
			return const_iterator(content.cbegin().base());
		}
		const_iterator begin() const
		{
			return const_iterator(cbegin().base());
		}
		const_iterator cend() const
		{
			return const_iterator(content.cend().base());
		}
		...
		void insert(InputIt first, InputIt last, Collision &&collision)
		{
			...
			sort();
			auto l = filter_unique(content.begin(), content.end(), std::forward<Collision>(collision)); //pack the unique element at the begining, mainting order
		...
		void insert(const_iterator first, const_iterator last, Collision &&collision, NoCollision &&nocollision)
		{
			...
			while (look != begin() and last != first)
			{
					...
					break; // we've copied everything, there's nothing to move.
				{
					...
					if (move_to_end != look ) std::move_backward(lb, look, move_to_end); //we don't want to do anything here if move_to_end == look. plus move_backward behavior is undefined in that case.
					if (found_collision)
						collision((move_to_end - 1)->second, last->second); // if we move the collision treatment here, we allow parallelism.
					move_to_end -= look - lb;
					look = lb;
		...
		void merge(const flat_map<key_type, mapped_type, C2, allocator_type> &source, Collision &&collision, NoCollision &&nocollision)
		{
			insert(source.begin(), source.end(), std::forward<Collision>(collision), std::forward<NoCollision>(nocollision));
		}
		...
		template <class C2, class Collision, class NoCollision>
		void merge(flat_map<key_type, mapped_type, C2, allocator_type> &&source, Collision &&collision, NoCollision &&nocollision)
		{
			if (source.capacity() >= source.size() + size()) //reuse source's resources if it avoid doing an allocation.
			{
				...
				insert(
					source.begin(), source.end(), [collision](auto &a, auto &b) {
	...
	*/
	...
	qtt_TEST_CASE("flat_map")
	{
		...
		{
			auto pre_ordered = flat_map<int, double>({{10, 1.4}, {20, 1.2}, {30, 1.3}, {40, 1.1}}); // allocation of the pre-ordered flatmap trigger a report from valgrind (invalid read)
		...
		qtt_SUBCASE("merge sequences, collision moment test")
		{
			...
			flat_map<int, double> result{{10, 120}, {20, 120}, {30, 120}, {33, 120}, {40, 120}, {1, 120}, {50, 120}, {70, 120}};
