...
namespace quantt
{
	...
	public:
		...
		MPT(MPT &&other)
		noexcept : vector_lift<MPT>(std::move(other)) {}
	 ...
	 */
		...
		MPS(std::initializer_list<Tens> initl, size_t oc = 0) : vector_lift<MPS>(initl), orthogonality_center(oc)
		{
			bool ok = check_ranks();
			if (not ok)
				throw std::invalid_argument("one or more input Tensors has rank differing from 3 and/or a bond dimension mismatch with its neighbor (dims 0 and 2).");
			if (oc >= size() and oc != 0)
		...
		MPS(size_type lenght, const Tens &val, size_t oc = 0) : vector_lift(lenght, val), orthogonality_center(oc)
		{
			...
			if (oc >= size() and oc != 0)
				throw std::invalid_argument("orthogonality center position greater than the number of defined tensors.");
			if (not ok)
				throw std::invalid_argument("Input tensor must be of rank 3 and have equal bond dimensions (dims 0 and 2).");
		}
		...
		MPS(const MPT &other, size_t oc = 0) : vector_lift<MPS>(other), orthogonality_center(oc)
		{
			bool ok = check_ranks();
			if (not ok)
				throw std::invalid_argument("Input MPT is an invalid MPS: one or more Tensors has rank differing from 3 and/or a bond dimension mismatch with its neighbor (dims 0 and 2).");
			if (oc >= size() and oc != 0)
				throw std::invalid_argument("orthogonality center position greater than the number of defined tensors.");
		}
	...
	public:
		...
		MPO(MPO &&other)
		noexcept : vector_lift<MPO>(std::move(other)) {}
		MPO(std::initializer_list<Tens> initl) : vector_lift<MPO>(initl)
		{
			bool ok = check_ranks();
			if (not ok)
				throw std::invalid_argument("one or more input Tensors has rank differing from 4 and/or a bond dimension mismatch with its neighbor (dims 0 and 2).");
		}
		...
		MPO(size_type size, const Tens &val) : vector_lift<MPO>(size, val)
		{
			bool ok = check_one(val);
			if (not ok)
				throw std::invalid_argument("Input tensor must be of rank 4 and have equal bond dimensions (dims 0 and 2).");
		}
		...
		MPO(const MPT &other) : vector_lift<MPO>(other)
		{
			bool ok = check_ranks();
			if (not ok)
				throw std::invalid_argument("Input MPT is an invalid MPO: one or more Tensors has rank differing from 4 and/or a bond dimension mismatch with its neighbor (dims 0 and 2).");
		}
	...
	template <class T, class Z = std::enable_if_t<std::is_base_of_v<vector_lift<T>, T>>> // Z only serves to prevent call on something else than MPT,MPS and MPO.
