...
torch::Scalar details::dmrg_impl(const MPO& hamiltonian,const MPT& twosites_hamil, MPS& in_out_state,const dmrg_options& options, env_holder& Env)
{
	...
	if (twosites_hamil.size()==1) step = 0;
	// fmt::print("step {}\n",step);
...
std::tuple<torch::Tensor,torch::Tensor,torch::Tensor> details::eig2x2Mat(torch::Tensor a0,torch::Tensor a1, torch::Tensor b)
{
	...
	auto E1 = (a0+a1 + crit)/2;
	auto o_coeff = torch::sqrt( (E0-a1)/(E0-E1)); // from arxiv.org/pdf/1908.03795.pdf
...
std::tuple<torch::Tensor,torch::Tensor,torch::Tensor,torch::Tensor> details::one_step_lanczos(torch::Tensor state,torch::Tensor hamil,torch::Tensor Lenv,torch::Tensor Renv)
{
	...
	if (torch::ge(b.abs(),1e-15).item().to<bool>()) psi_ip /= b;
